...
{
	/// <summary>
	/// Wraps a float between 0 and wrapValue
	/// </summary>
	/// <returns>
	/// The wrapped float.
	/// </returns>
	/// <param name='value'>
	/// Value you want to wrap
	/// </param>
	/// <param name='wrapValue'>
	/// The maximum amount the value can be
	/// </param>
	public static float WrapFloat(float value, float wrapValue, bool posOnly = true)
	{
		float retVal = value % wrapValue;
		if(posOnly && retVal < 0) retVal += wrapValue;
		return retVal;
	}
	...
	/// </returns>
	/// <param name='angle'>
	/// Angle you want to wrap
	/// </param>
	public static float WrapAngle(float angle)
	{
		return WrapFloat( angle, 360f);
	}
	
	/// <summary>
	/// Simplified angle clamp to clamp an angle between -Val and +Val
	/// Flips direction at 180 degrees
	/// </summary>
	/// <returns>
	/// Clamped angle in [0..360]
	/// </returns>
	/// <param name='angle'>
	/// Angle in degrees, can be outside [0..360] range
	/// </param>
	/// <param name='min'>
	/// Minimum in [-180..0] range
	/// </param>
	/// <param name='max'>
	/// Maximum in [0..180] range
	/// </param>
	public static float ClampAngleAroundOrigin(float angle, float min, float max)
	{
		angle = WrapAngle(angle);
		if(angle > 180.0f) angle -= 360.0f;
		return WrapAngle( Mathf.Clamp (angle, min, max));
	}
	...
	public static float PositionToAngle(float position, float circumference)
	{
		return position / circumference * 360f;
	}
	
	public static float AngleToPosition(float angle, float circumference)
	{
		return angle / 360f * circumference;
	}
	...
	/// <summary>
	/// 2D line intersection, lines have infinite length (no segments)
	/// </summary>
	/// <returns><c>true</c>, if line intersection is valid, <c>false</c> if lines are parallel</returns>
	public static bool LineLineIntersection(out Vector2 intersection, Vector2 s1, Vector2 e1, Vector2 s2, Vector2 e2)
	{
		intersection = Vector2.zero;

		// Get A,B,C of first line - points : s1 to e1
		float A1 = e1.y-s1.y;
		float B1 = s1.x-e1.x;
	...
	//"point" is assumed to be on a line created by linePoint1 and linePoint2.
	//Returns 0 if point is on the line segment.
	//Returns 1 if point is outside of the line segment and located on the other side of linePoint2.
